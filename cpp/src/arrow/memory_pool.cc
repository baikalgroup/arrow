...
namespace arrow {
DEFINE_int64(refuse_if_exceed_max_malloc_gb, -1, "refuse_if_exceed_max_malloc_gb");
DEFINE_int64(refuse_big_size_mb, 4, "refuse_big_size_mb");
...
class SystemMemoryPoolWithLimit : public BaseMemoryPoolImpl<SystemAllocator> {
 public:
  ...
  Status check_need_refuse(int64_t alloc_size) {
    if (FLAGS_refuse_if_exceed_max_malloc_gb <= 0
        || alloc_size <= 0) {
      return Status::OK();
    }
    int64_t all_die_limit = FLAGS_refuse_if_exceed_max_malloc_gb * 1024 * 1024 * 1024LL;
    ...
    if (new_total_alloc_size > big_die_limit && alloc_size > FLAGS_refuse_big_size_mb * 1024 * 1024LL) {
